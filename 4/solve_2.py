...
def solve(data):
    ...
    for guard_id, asleep_minutes in asleep_map.items():
        value_counts = pd.Series(asleep_minutes).value_counts()
        asleep_minutes = value_counts.max()

        if asleep_minutes > max_asleep_minutes:
            max_minute = value_counts.idxmax()
